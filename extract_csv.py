...
# Define the camera name and CSV file path
# cameraName = 'test8_ANPHU17' 
# cameraName = 'test8_BINHAN5' 
cameraName = 'ANPHU17' 
csv_file_path = f"{cameraName}.csv"

# Read the CSV file into a DataFrame
df = pd.read_csv(csv_file_path)

# Calculate total vehicles (car and motorcycle)
total_vehicle = df[df['label'].isin(['car', 'motorcycle'])].shape[0]

# Calculate total plates
total_plate = df[df['label'] == 'plate'].shape[0]

# Initialize Plate_Checker and count passing plates
c = 0
checker = Plate_Checker()
for ocr in df['plate']:
    if ocr != "nan" and isinstance(ocr, str):
        if checker.run(ocr.split('\"')[1])[1]:
            c += 1
        else:
            print('Plate False: ', ocr.split('\"')[1])
print()

# Print the results
print('cameraName', cameraName)
print("Total Vehicles (car and motorcycle):", total_vehicle)
print("Total Plates:", total_plate)
print("Plates Passed:", c)